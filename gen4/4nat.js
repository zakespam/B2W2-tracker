...
function uploadLocalStorage(event, key = '4natKey') {
...
function downloadLocalStorage(key = '4natKey') {
...
document.addEventListener("DOMContentLoaded", () => {
  fetch('gen4nat.json')
    ...
    .then(pokedexData => {
      ...
      window.caught = JSON.parse(localStorage.getItem("4natKey") || "[]");
        ...
        for (let b = 0; b < boxes; b++) {
          ...
          pokedexData.slice(b * 30, b * 30 + 30).forEach(p => {
            ...
            tile.addEventListener("click", () => {
              ...
              localStorage.setItem("4natKey", JSON.stringify(caught));
              updateProgress();
            });
      ...
      document.getElementById("reset-all").addEventListener("click", () => {
        caught = [];
        localStorage.removeItem("4natKey");
        renderPokedex();
        updateProgress();
      });
