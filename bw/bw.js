...
function uploadLocalStorage(event, key = 'bwKey') {
...
function downloadLocalStorage(key = 'bwKey') {
...
document.addEventListener("DOMContentLoaded", () => {
  fetch('bw.json')
    ...
    .then(pokedexData => {
      ...
      window.caught = JSON.parse(localStorage.getItem("bwKey") || "[]");
        ...
        for (let b = 0; b < boxes; b++) {
          ...
          pokedexData.slice(b * 30, b * 30 + 30).forEach(p => {
            ...
            tile.addEventListener("click", () => {
              ...
              localStorage.setItem("bwKey", JSON.stringify(caught));
              updateProgress();
            });
