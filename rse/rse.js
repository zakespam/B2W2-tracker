...
function uploadLocalStorage(event, key = 'rseKey') {
...
function downloadLocalStorage(key = 'rseKey') {
...
document.addEventListener("DOMContentLoaded", () => {
  fetch('rse.json')
    ...
    .then(pokedexData => {
      ...
      window.caught = JSON.parse(localStorage.getItem("rseKey") || "[]");
        ...
        for (let b = 0; b < boxes; b++) {
          ...
          pokedexData.slice(b * 30, b * 30 + 30).forEach(p => {
            ...
            tile.addEventListener("click", () => {
              ...
              localStorage.setItem("rseKey", JSON.stringify(caught));
              updateProgress();
            });
      ...
      document.getElementById("reset-all").addEventListener("click", () => {
        caught = [];
        localStorage.removeItem("rseKey");
        renderPokedex();
        updateProgress();
      });
