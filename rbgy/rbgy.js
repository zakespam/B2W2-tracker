...
function uploadLocalStorage(event, key = 'rbgyKey') {
...
function downloadLocalStorage(key = 'rbgyKey') {
...
document.addEventListener("DOMContentLoaded", () => {
  fetch('rbgy.json')
    ...
    .then(pokedexData => {
      ...
      window.caught = JSON.parse(localStorage.getItem("rbgyKey") || "[]");
        ...
        for (let b = 0; b < boxes; b++) {
          ...
          pokedexData.slice(b * 30, b * 30 + 30).forEach(p => {
            ...
            tile.addEventListener("click", () => {
              ...
              localStorage.setItem("rbgyKey", JSON.stringify(caught));
              updateProgress();
            });
      ...
      document.getElementById("reset-all").addEventListener("click", () => {
        caught = [];
        localStorage.removeItem("rbgyKey");
        renderPokedex();
        updateProgress();
      });
