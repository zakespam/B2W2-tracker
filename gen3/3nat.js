...
function uploadLocalStorage(event, key = '3natKey') {
...
function downloadLocalStorage(key = '3natKey') {
...
document.addEventListener("DOMContentLoaded", () => {
  fetch('gen3nat.json')
    ...
    .then(pokedexData => {
      ...
      window.caught = JSON.parse(localStorage.getItem("3natKey") || "[]");
        ...
        for (let b = 0; b < boxes; b++) {
          ...
          pokedexData.slice(b * 30, b * 30 + 30).forEach(p => {
            ...
            tile.addEventListener("click", () => {
              ...
              localStorage.setItem("3natKey", JSON.stringify(caught));
              updateProgress();
            });
      ...
      document.getElementById("reset-all").addEventListener("click", () => {
        caught = [];
        localStorage.removeItem("3natKey");
        renderPokedex();
        updateProgress();
      });
